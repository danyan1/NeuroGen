...
def display_candidate_loss(scores, nx, ny, ns):
    ...
    s = scores.reshape((nx, ny, ns)).transpose((1, 0, 2))[::-1, :,
        :]  ## The transpose and flip is just so that the candidate
...
def plot_pretty_compare(X, Y, threshold, xlim, ylim, cmap='Blues'):
    ...
    ax1 = g.ax_marg_x.hist(x, log=True, color=color1, bins=30,
...
def plot_fwrf_paper_compare(X, Y, threshold, xlim, ylim, cmap='Blues'):
    ...
    g.ax_marg_x.text(0.35, 50., '%.2f' % (1. - adv), horizontalalignment='left', fontsize=18, color=color2,
                     weight='bold')
