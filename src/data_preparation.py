...
def dnn_feature_extractor(_incoming, param_file_name='', drop_ratio_conv=0., drop_ratio_fc=0.5):
    ...
    # block 1
    conv1 = conv(l_in, num_filters=96, filter_size=7, stride=2, pad=0, W=I.Normal(.02), b=I.Constant(0),
                 nonlinearity=NL.rectify)  # 111
    ...
    # block 2
    conv2 = conv(lrn1, num_filters=256, filter_size=5, stride=2, pad=1, W=I.Normal(.02), b=I.Constant(1),
                 nonlinearity=NL.rectify)  # 27
    ...
    # block 3
    conv3 = batch_norm_n(conv(lrn2, num_filters=384, filter_size=3, stride=1, pad=1, W=I.Normal(.02), b=I.Constant(0),
                              nonlinearity=NL.rectify))  # 13
    # block 4    
    drop4 = L.DropoutLayer(conv3, p=drop_ratio_conv, rescale=True)
    conv4 = batch_norm_n(conv(drop4, num_filters=384, filter_size=3, stride=1, pad=1, W=I.Normal(.02), b=I.Constant(1),
                              nonlinearity=NL.rectify))  # 11
    # block 5      
    drop5 = L.DropoutLayer(conv4, p=drop_ratio_conv, rescale=True)
    conv5 = batch_norm_n(conv(drop5, num_filters=256, filter_size=3, stride=1, pad=1, W=I.Normal(.02), b=I.Constant(0),
                              nonlinearity=NL.rectify))  # 9
...
def create_gabor_feature_maps(stim_data, gabor_params, nonlinearity=lambda x: x):
    ...
    #
    fmaps, fmaps_sizes = preprocess_gabor_feature_maps(gfm.create_feature_maps(stim_data), act_func=nonlinearity,
                                                       dtype=np.float32)
    ...
    for r in fmaps_sizes:
        partitions += [partitions[-1] + r[1], ]
    freq_rlist = [range(start, stop) for start, stop in
                  zip(partitions[:-1], partitions[1:])]  # the frequency ranges list
