...
def make_feature_table(num_orientations, cycles_per_stim,
    ...
    assert isinstance(cycles_per_stim, (tuple, list,
                                        np.ndarray)), 'cycles_per_stim must either be a tuple of len 3 (min, max, num), or an explicit list of spatial freqs (np array or list)'
    ...
    if type(cycles_per_stim) is tuple:
        ...
        if freq_spacing == 'log':
            cycles_per_stim = np.logspace(np.log10(cycles_per_stim[0]), np.log10(cycles_per_stim[1]),
                                          num=cycles_per_stim[2])
...
def make_filter_stack(feature_table, pix_per_filter, cycles_per_filter, envelope_radius_pix, complex_cell=True,
                      color_channels=1):
    ##initialize
...
class Gaborizer(nn.Module):
    ...
    def __init__(self, num_orientations, cycles_per_stim,
        ...
        if complex_cell:
            self.real_filters_tnsr = nn.Parameter(
                torch.tensor(np.real(uniq_filters), dtype=torch.float32, requires_grad=True))
            self.imag_filters_tnsr = nn.Parameter(
                torch.tensor(np.imag(uniq_filters), dtype=torch.float32, requires_grad=True))
        else:
...
class GaborFeatExtractor(nn.Module):
    ...
    def forward(self, minibatch_stim_tnsr):
        ...
        if self.pad_type is 'half':
            pad_sz = self.real_filters_tnsr.shape[
                         2] // 2  # pad input w/ sym. border that is the floor the filter width/2
        else:
