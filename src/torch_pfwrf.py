...
class Torch_fwRF_voxel_block(nn.Module):
    def __init__(self, _fmaps_fn, _nonlinearity=None, \
        ...
        if shared_fpf:
            ...
            self.fpf_rez += [nr, ]
            self.rfs += [nn.Parameter(
                torch.tensor(np.ones(shape=(nv, nr, nr), dtype=np.float32), requires_grad=True).to(device)), ]
            ...
            for k, fm_rez in enumerate(self.fmaps_rez):
                ...
                if nr == 1:
                    self.rfs += [nn.Parameter(
                        torch.tensor(np.ones(shape=(nv, nr, nr), dtype=np.float32), requires_grad=False).to(device)), ]
        ...
        self.w = nn.Parameter(
            torch.tensor(np.random.normal(0, 0.001, size=(nv, nf)).astype(dtype=np.float32), requires_grad=True).to(
                device))
        self.b = nn.Parameter(
            torch.tensor(np.full(fill_value=0.0, shape=(nv,), dtype=np.float32), requires_grad=True).to(device))
    ...
    def forward(self, _fmaps):
        ...
        if len(self.rfs) == 1:
            for fm, d, u in zip(_fmaps, self.dl, self.ul):
                ...
                phi += [torch.tensordot(g, f, dims=[[1], [
                    2]]), ]  # apply pooling field and add to list. phi : [nv, batch, features]
        else:
        ...
        _r = torch.squeeze(torch.bmm(Phi, torch.unsqueeze(self.w, 2))).t() + torch.unsqueeze(self.b,
                                                                                             0)  # _r : [batch, nv]
        return _r
...
def learn_params_gradient(data, voxels, _fmaps_fn, _fwrf_fn, \
    ...
    for rv, lv in iterate_range(0, nv, voxel_batch_size):
        ...
        if lv < voxel_batch_size:
            voxel_batch = np.concatenate([voxel_batch, np.zeros(shape=(nt, voxel_batch_size - lv), dtype=dtype)],
                                         axis=1)
        # reset param block
        ...
        for epoch in range(num_epochs):
            ...
            sys.stdout.write('\rvoxels [%6d:%-6d] of %d, epoch %3d of %3d, loss = %.6f (%d)' % (
            rv[0], rv[-1], nv, epoch + 1, num_epochs, np.mean(cur_losses), np.sum(improvement)))
    sys.stdout.flush()
